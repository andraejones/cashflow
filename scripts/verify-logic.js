...
// TEST 102: pass 2 (near amount) exists for exactly one thing — the same
// purchase logged a few cents off — and its own conflict guard was rejecting
// it. The guard reads RAW description tokens and asks whether each side
// carries a distinctive word the other lacks. A bank line keeps no merchant
// token in raw form once the length floor and the stopword list are applied
// ("WM SUPERCENTER #778 5001 TAYLOR RD PUNTA" -> TAYLOR, PUNTA: pure location
// noise), so against a plain "Walmart" entry both sides had an unshared word
// and a $16.57-for-$16.67 typo was declared a different charge. It then showed
// up TWICE in the report — once as a bank line missing from the app, once as
// an app entry with no bank record — with no way to reconcile the two.
console.log("TEST 102: A Cents-Off Typo Pairs For Review, A Different Product Still Doesn't");
{
  const run = (csv, seed) => {
    const s = new TransactionStore();
    s.resetData();
  ...
  // One exact match and one cents-off typo, same payee, same day.
  const walmartCsv =
    "Posted Date,Transaction Date,Description,Deposit,Withdrawal,Balance\n" +
    "8/21/2026,8/21/2026,Withdrawal POS #623300530533 WM SUPERCENTER #778 5001 TAYLOR RD PUNTA Card 4754,,($16.67),$2121.65\n" +
    "8/21/2026,8/21/2026,Withdrawal POS #623300166386 WM SUPERCENTER #778 5001 TAYLOR RD PUNTA Card 4754,,($13.49),$2303.32\n";
  const walmart = run(walmartCsv, (s) => {
    s.addTransaction("2026-08-21", {
      amount: 13.49, type: "expense", description: "Walmart", settled: true,
    });
    s.addTransaction("2026-08-21", {
      amount: 16.57, type: "expense", description: "Walmart", settled: true,
    });
  });

  if (walmart.reviewPairs.length !== 1) {
    throw new Error(
      `The cents-off typo must pair for review, got ${walmart.reviewPairs.length} pairs`
    );
  }
  const pair = walmart.reviewPairs[0];
  if (Math.abs(pair.bank.signed + 16.67) > 0.005 || Math.abs(pair.app.amount - 16.57) > 0.005) {
    throw new Error(`Wrong pair: ${JSON.stringify({ bank: pair.bank.signed, app: pair.app.amount })}`);
  }
  // The whole point: one purchase, reported once. Not once per side.
  if (walmart.missingFromApp.length !== 0) {
    throw new Error(
      `The typo's bank line is still reported as missing: ${JSON.stringify(walmart.missingFromApp)}`
    );
  }
  if (walmart.appOnlyUnmatched.length !== 0) {
    throw new Error(
      `The typo's app entry is still reported as unmatched: ${JSON.stringify(walmart.appOnlyUnmatched)}`
    );
  }

  // Negative control: narrowing the guard must not gut it. Same merchant after
  // normalization, near amount, same day — but each raw description names a
  // product the other lacks (a MKTPLACE order vs a PRIME renewal), which is
  // positive evidence of two different charges.
  //
  // The three exact-matching Amazon lines are load-bearing: they push AMAZON
  // over distinctiveMaxDocFreq so pass 3 (name-assisted) can't reach for it,
  // leaving pass 2 and its conflict guard as the only thing that could pair
  // the near amounts. Without them AMAZON reads as a rare word, pass 3 pairs
  // the two on the name alone, and this control passes no matter what pass 2
  // does.
  const amazonLine = (day, amt, tail) =>
    `8/${day}/2026,8/${day}/2026,Withdrawal Debit Card AMAZON ${tail} WA Card 4754,,($${amt}),$2300.04\n`;
  const amazonCsv =
    "Posted Date,Transaction Date,Description,Deposit,Withdrawal,Balance\n" +
    amazonLine(18, "14.97", "MKTPLACE 5A6QG31H1 Amzn.com/bill") +
    amazonLine(18, "40.00", "RETAIL 5A6QG31H2 Amzn.com/bill") +
    amazonLine(17, "50.00", "RETAIL 5A6QG31H3 Amzn.com/bill") +
    amazonLine(16, "60.00", "RETAIL 5A6QG31H4 Amzn.com/bill");
  const amazon = run(amazonCsv, (s) => {
    s.addTransaction("2026-08-18", {
      amount: 15.16, type: "expense", description: "Amazon Prime", settled: true,
    });
    [["2026-08-18", 40], ["2026-08-17", 50], ["2026-08-16", 60]].forEach(([d, amt]) =>
      s.addTransaction(d, { amount: amt, type: "expense", description: "Amazon", settled: true })
    );
  });
  if (amazon.reviewPairs.length !== 0) {
    throw new Error(
      `Different products on the same merchant must not pair: ${JSON.stringify(
        amazon.reviewPairs.map((p) => [p.bank.signed, p.app.amount])
      )}`
    );
  }
  if (amazon.missingFromApp.length !== 1 || amazon.appOnlyUnmatched.length !== 1) {
    throw new Error("The unpaired Amazon lines must both still be reported");
  }

  console.log("✅ A cents-off typo reconciles as one review pair; a product conflict still doesn't");
}

...
- Debt snowball planner (a full-page view) with minimum-plus-extra payment scheduling, a per-debt interest rate field, a configurable start point for the extra payment, one-time cash infusions, remaining-balance display on each day-view debt payment, and a "Convert to Debt" shortcut from any recurring expense.

...
- Debt snowball planner (a full-page view): every debt pays only its minimum each month while the extra (base extra + freed-up minimums of paid-off debts) accumulates into a set-aside fund; a debt is paid off in a single lump sum the first month the fund can cover its remaining balance, smallest-balance first. (This suits lenders that apply overpayments as pre-payments rather than to principal.) Includes a per-debt interest rate field, a configurable start point for the extra payment, one-time cash infusions, remaining-balance display on each day-view debt payment, and a "Convert to Debt" shortcut from any recurring expense.
